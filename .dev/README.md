...
- ⬜ «Повні дані»: план [T10](.t/T10-P--full-data-plan.md) (rsync хвоста зі старого сервера → доімпорт → адаптер для `_old_data.zip` (7 656 версій 2024-03/04) → перевірки), питання [T09](.t/T09-Q--old-server-data.md). Перевірено: subjects main/light байт-у-байт однакові, різниця лише в тому, хто зловив версію — обʼєднання в глобальній `history` і є чиста база.
...
- Ти: відповіді `>` у [T09](.t/T09-Q--old-server-data.md) (де cron, хто виконує, куди rsync, коли вимикати) → далі кроки T10 по черзі.
